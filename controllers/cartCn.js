...
export const  addToCart = catchAsync(async(req, res, next) => { 
    const {name,price,discountPrice,images,shopkeeperId,quantity}=await Product.findById(req.body.productId)
    const {id}=jwt.verify(req.headers['authorization'].split(' ')[1],process.env.JWT_SECRET)
    if(req.body.productId && req.body.quantity){
        ...
        if(!productInCart){
            ...
            newCart.push({productId:req.body.productId,name,price,discountPrice,images,shopkeeperId,quantity:req.body.quantity})
        }
        const newCartUser=await User.findByIdAndUpdate(id,{cart:newCart},{new:true})
